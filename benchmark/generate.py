...
RUN_DEFINITION_TEMPLATE_TCE = """  <rundefinition name=\"COUNT\">
  <requiredfiles>MUTANT</requiredfiles>
  <option name=\"benchmark/MUTANT\"/>
  <tasks>
    <include>ORIG</include>
  </tasks>
  </rundefinition>"""
...
def find_yaml_for_original(original_path: str) -> str:
    ...
    for candidate in candidates:
        ...
        if original_name in content or relative_original in content:
            rel_candidate = candidate.relative_to(
                Path(__file__).parent / Path(ORIGINAL_PREFIX)
            )
...
def create_run_definitions(csv_path: Path, limit: int, template: str) -> str:
    ...
    with csv_path.open(newline="", encoding="utf-8-sig") as handle:
        ...
        index_by_name = {
            normalized: idx
            for idx, normalized in enumerate(normalized_fieldnames)
            if normalized
        }
        ...
        for count, row in enumerate(reader, start=1):
            ...
            block = (
                template.replace("COUNT", f"mutant_{count}")
...
def inject_into_template(
    template_content: str, definitions_block: str, options: list[str]
) -> list[str]:
    """Insert generated run definitions directly after the DEFINITIONS marker."""
    final_contents = []

    for option in options:
        curr_template_content = template_content.replace("<!-- OPTION -->", option)
        marker_index = curr_template_content.find(DEFINITIONS_MARKER)
        if marker_index == -1:
            raise ValueError(f"Could not find marker in template: {DEFINITIONS_MARKER}")
        insert_at = marker_index + len(DEFINITIONS_MARKER)
        injection = f"\n\n{definitions_block}"
        final_contents.append(
            curr_template_content[:insert_at]
            + injection
            + curr_template_content[insert_at:]
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Limit number of mutants per operator. To use all, set -1.",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "--tce",
        action="store_true",
        help="Check whether tce should be generated",
    )
    ...
    definitions = create_run_definitions(args.csv, args.limit, RUN_DEFINITION_TEMPLATE_TCE if args.tce else RUN_DEFINITION_TEMPLATE)
    template_content = args.template.read_text(encoding="utf-8")
    template_content = template_content.replace("CPUMODEL", args.cpu_model)
    final_contents = inject_into_template(
        template_content,
        definitions,
        [""]
        if not args.tce
        else [
            '<option name="gcc">-O1</option>',
            '<option name="gcc">-O2</option>',
            '<option name="gcc">-O3</option>',
            '<option name="gcc">-Os</option>',
            '<option name="clang">-O1</option>',
            '<option name="clang">-O2</option>',
            '<option name="clang">-O3</option>',
            '<option name="clang">-Os</option>',
        ],
    )

    output_path = args.output if args.output is not None else args.template
    for i, final_content in enumerate(final_contents):
        if len(final_contents) > 1:
            name = f"{i}_{args.output.name}"
            if args.tce:
                name = f"tce_{name}"
            output_path = args.output.parent / name
